...
Flask keep-alive (for Render + UptimeRobot) + Telegram bot (polling), one file, one process.

Start command on Render:
    python app.py
(or: gunicorn app:app --workers 1 --threads 4 --timeout 120  — workers MUST be 1)
...
# ─────────────────────────────────────────────────────────────
# IMPORT YOUR MODULES (with safe dummy fallback)
# ─────────────────────────────────────────────────────────────
try:
    ...
    class DummyTrader:
        def get_sol_balance(self): return 0.0
        def get_token_balance(self, m): return {"ui": 0, "raw": 0, "decimals": 0}
        def sell_token(self, m, a): return "dummy"
        def buy_token(self, m, a): return "dummy"

    class DummyMgr:
        def create_mint(self): return ("dummy", "dummy")
        def mint_to_wallet(self, m, a): return "dummy"

    class DummyAnalytics:
        def get_holder_count(self, m): return 0
        def get_token_price(self, m): return 0.0

    class DummyWallet:
        solana_pubkey = "NotConfigured"
        eth_address = "NotConfigured"
        seed_phrase = None
        def get_all_addresses(self): return {"solana": "N/A", "ethereum": "N/A"}

    wallet = DummyWallet()
    trader = DummyTrader()
    token_mgr = DummyMgr()
    analytics = DummyAnalytics()
    liq_mgr = type("obj", (object,), {"find_pools": lambda self, m: []})()
    volume_engines = {}
    VolumeEngine = lambda *a, **k: None
...
def _run_bot() -> None:
    ...
    try:
        ...
        logger.info(f"[BOT] Token loaded: True | Imports OK: {IMPORTS_OK}")
        ...
        # stop_signals=None is REQUIRED — default tries to install signal
...
def _start_bot_once() -> None:
    """Launch the bot exactly once, whether started via `python app.py`
    or imported by gunicorn."""
...
# Fires under both `python app.py` and `gunicorn app:app`
_start_bot_once()
...
if __name__ == "__main__":
    ...
    # use_reloader=False prevents the bot from being launched twice in dev
